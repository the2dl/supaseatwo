...
def ad_health(domain_controller, username, password, output_file):
    ...
    def ad_health_operation():
        ...
        logger.debug(f"Found {len(conn.entries)} groups")
        for entry in conn.entries:
            group = {
        ...
        conn.unbind()
        return f"Successfully retrieved AD information and wrote it to {output_file}. Users: {len(users)}, Groups: {len(groups)}"
